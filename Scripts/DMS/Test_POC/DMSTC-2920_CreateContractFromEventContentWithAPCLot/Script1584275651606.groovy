...
import aribaui.util.AribaUIHelper
...
List<String> registeredToggles = register.registerTogglesAndGetActualToEnable(testData.getTestDataForGivenColumn(toggles));
List<String> registeredParams = register.registerParametersAndGetActualToEnable(testData.getTestDataForGivenColumn(params));

//Set Toggles
for(int i=0; i< registeredToggles.size(); i++){
	WebUI.callTestCase(findTestCase('DMS/UtilTests/SetFeatureToggle'), [('featureToggleName') : registeredToggles[i]],
		FailureHandling.STOP_ON_FAILURE)
}
...
for(int i=0; i< registeredParams.size(); i++){
	WebUI.callTestCase(findTestCase('DMS/UtilTests/SetRealmParameters'), [('parameterName') : registeredParams[i]],
		FailureHandling.STOP_ON_FAILURE)
}
...
/*TestObject suplrTab = Ariba.createTestObject("//a[@awname='participants' and text()='Suppliers']")
Ariba.aribaClick(suplrTab)*/
