...
public class ExcelTestDataUtil {
	...
	public ExcelTestDataUtil(String sheetName, String dataTableName){
		this.dataTableName = dataTableName;
		util = new ExcelInit(testDataFileFullPath.trim(), sheetName);
	}


	@Keyword
	public Map<String,String> getCellValuesOfRow(int rowNum) throws IOException, NoSuchFieldException {
	...
	public String getCellValueForKey(String key) throws NoSuchFieldException {
		...
		for(int i=startCellNum; i<endCellNum; i++){
			...
			/*Cell cellh = util.getSheetObj().getRow(rowStart-1).getCell(i);
			cellh.setCellType(Cell.CELL_TYPE_STRING);*/
			String cellheader = convertNumericToStringCellValue(util.getSheetObj().getRow(rowStart-1).getCell(i));
			String cellVal = convertNumericToStringCellValue(util.getSheetObj().getRow(rowStart).getCell(i));
			/*Cell cellV = util.getSheetObj().getRow(rowStart).getCell(i);
			cellV.setCellType(Cell.CELL_TYPE_STRING);
			String cellVal = cellV.getStringCellValue();*/
			/*String cellVal = "";
			if(DateUtil.isCellDateFormatted(cellV)){
				Date date = cellV.getDateCellValue();
				cellVal = date.toString();
			}*/
	...
	private String convertNumericToStringCellValue(Cell cell){
		cell.setCellType(Cell.CELL_TYPE_STRING);
		return cell.getStringCellValue();
	}
	

	public List<Map<String,String>> getAllDataFromAllRows() throws NoSuchFieldException, IOException {
		//String file = "TestData.xlsx";
		//String fileFullPath = "/Users/i331477/git/S4MicroServiceKatalon/Test Files/dms/ExcelValidations/" + file;
		//util = new ExcelInit(testDataFileFullPath.trim(), sheet);
		List<String> cellValues = null;
		...
		for(int i=rowStart; i< rowEnd; i++){
			...
			for(int j=startCellNum; j<endCellNum; j++){
				...
				if(null != util.getSheetObj().getRow(rowStart-1).getCell(j)){
					...
					cellHeader = convertNumericToStringCellValue(util.getSheetObj().getRow(rowStart-1).getCell(j));
					/*Cell cellH = util.getSheetObj().getRow(rowStart-1).getCell(j);
					cellH.setCellType(Cell.CELL_TYPE_STRING);
					cellHeader = cellH.getStringCellValue();*/
				}

				if(null != util.getSheetObj().getRow(i).getCell(j)){
					cellVal = convertNumericToStringCellValue(util.getSheetObj().getRow(i).getCell(j));
					
					/*Cell cellV = util.getSheetObj().getRow(i).getCell(j);
					cellV.setCellType(Cell.CELL_TYPE_STRING);
					cellVal = cellV.getStringCellValue();*/
				}
	 ...
	 */
	@Keyword
	public List<String> getTestDataForGivenColumn(String columnHeader){
	...
	private List<Row> getRowsInsideTableTag(XSSFSheet sheet, String dataTableName){
	...
	private List<String> getAllValueForColumn(List<Row> rowList, String columnHeader) {
