...
public class ExcelToggleParamStoreStrategyDummy {
	...
	ToggleParamSetterPojo uniqToggles = null;


	public ExcelToggleParamStoreStrategyDummy() throws IOException {
	...
	public synchronized List<String> registerTogglesAndGetActualToEnable(List<String> toggles) throws IOException {
		uniqToggles = new ToggleParamSetterPojo();
		initExcelObject();
		//resetToggleToBeSet();
		...
		writeTogglesIntoExcel(existingToggles, uniqToggles.getTogglesToBeSet());
		...
		//return uniqToggles.getTogglesToBeSet();
	}
	...
	private List<String> getTotalExistingTogglesComparingNewToggles(List<String> existingToggles, List<String> newToggles) {
		...
		if (existingToggles.size() == 0 || existingToggles == null) {
			uniqToggles.setToggles(newToggles)
			return uniqToggles.getTogglesToBeSet();
		}
		...
		uniqToggles.setToggles(toggles);
	...
	public List<String> getTogglesTobeSetOrUnset() {
		...
		localList.clear();
		if(uniqToggles.getTogglesToBeSet().size()>0){
			for(int i=0; i< uniqToggles.getTogglesToBeSet().size(); i++){
				localList.add(uniqToggles.getTogglesToBeSet().get(i).split(":")[0]);
			}
			//togglesToBeSet = null;
